...
function App() {
  ...
  useEffect(() => {
    ...
    if (tokens.length === 1) {
      ...
      const nextToken = result.get(true)[0]["end"];
      const result2 = Map.groupBy(guaMap, ({ start }) => start === nextToken);
      setShowGuas(result2.get(true));
    }
    if (tokens.length === 2) {
      const result1 = Map.groupBy(
        guaMap,
        ({ name }) => name === tokens[0].slice(0, tokens[0].length - 1)
      );
      const result2 = Map.groupBy(
        guaMap,
        ({ name }) => name === tokens[1].slice(0, tokens[1].length - 1)
      );
      const startToken = result1.get(true)[0]["start"];
      const nextToken = result2.get(true)[0]["end"];
      const result3 = Map.groupBy(
        guaMap,
        ({ start, end }) => start === nextToken && end === startToken
      );
      setShowGuas(result3.get(true));
    }
    if (tokens.length === 3) {
      setShowGuas([{ str: "清空" }]);
    }
